...
7. For Next.js App Router or RSC projects, read `rsc-next-app-router-notes.md` before choosing data and Suspense boundaries.
8. Add feature-local `<Feature>.api.ts`, `<Feature>.service.ts`, and hook layers where the project uses explicit transport code.
9. Add optional feature files only when they clarify a real boundary.
10. Reject role-only feature-root filenames such as `api.ts`, `schema.ts`, `type.ts`, and `types.ts`.
11. Add `--with-hooks` when the project should receive SessionStart sync hooks.
12. Add `--with-docs` only when the project wants starter `ARCHITECTURE.md` and `docs/coding-patterns.md` scaffolds.
13. Record project-specific commands, routing setup, feature map, and exceptions in local docs.
...
6. For RSC projects, move page-wide loader-shaped data fetching into feature-owned server components or query/server boundaries where appropriate.
7. Isolate browser-only side effects behind feature-owned stores or adapters when such side effects exist.
8. Rename role-only feature-root files to `<Feature>.<role>.ts` unless local docs intentionally define an exception.
...
8. For Next.js App Router or RSC projects, use `rsc-next-app-router-notes.md` for server component, Suspense, action, and cache boundary decisions.
9. Keep local docs concise and put detailed reusable guidance in the installed skill references.
10. Avoid adding `widgets` unless repeated cross-feature blocks justify it.
