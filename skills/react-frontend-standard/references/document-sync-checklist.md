...
- Feature-root role files use `<Feature>.<role>.ts`; role-only filenames such as `api.ts`, `schema.ts`, `type.ts`, `types.ts`, `action.ts`, and `actions.ts` are renamed or documented as intentional local exceptions.
